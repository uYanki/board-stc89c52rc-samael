...
- 数据存贮器地址
    ...
    第二行: 0x40 ~ 0x50
- 显示自定义字符
    其内的CGRAM可储存8个自定义字符(像素5*7),使用前需使用LCD1602_WriteCmd(addr)把字模写入至CGRAM
    每个字符对应的首地址: addr = 0x40,0x48,0x50,0x58,0x60,0x68,0x70,0x78
    再通过 LCD1602_WriteData(ch) 将字符显示到屏幕(ddram)上, 0x00<ch<0x07
*/
...
void LCD1602_Off() { LCD1602_WriteCmd(0x08); } /*关闭显示屏 注:关闭后背光仍然会亮,但是不会再显示字符了*/
...
uint8_t code cgram[8] = {0x40, 0x48, 0x50, 0x58, 0x60, 0x68, 0x70, 0x78};

void LCD1602_WriteCGRAM(uint8_t pos /* pos of cgram:0~7 */, uint8_t* buf /* 一维数组 */) {
    uint8_t i;
    LCD1602_WriteCmd(cgram[pos]);
    for (i = 0; i < 8; ++i)  //
...
void LCD1602_WriteCGRAMs(uint8_t (*buf)[8] /* 二维数组 */, uint8_t len) {
    ...
    if (len > 8) len = 8;
    for (i = 0; i < len; ++i) {
        LCD1602_WriteCmd(cgram[i]);
        for (j = 0; j < 8; ++j)  //
            LCD1602_WriteData(buf[i][j]);
    }
}
